...
export function ClienteDetalhePage(): JSX.Element {
  ...
  if (clienteQuery.isError || !clienteQuery.data) {
    return (
      <Card>
        <CardContent className="p-6 text-sm text-destructive">Cliente não encontrado.</CardContent>
      </Card>
    );
  }
  ...
  return (
    <section className="space-y-4">
      <PageTitle
        title={cliente.nome}
        description="Dados completos e histórico resumido de compras"
        actions={
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <p className="md:col-span-2"><strong>Endereço:</strong> {cliente.endereco || "-"}</p>
          <p className="md:col-span-2"><strong>Observações:</strong> {cliente.observacoes || "-"}</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          ...
          <p><strong>Última compra:</strong> {cliente.resumo_compras.ultima_compra_em || "-"}</p>
        </CardContent>
      </Card>
    </section>
  );
