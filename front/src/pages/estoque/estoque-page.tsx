...
export function EstoquePage(): JSX.Element {
  ...
  const estoqueQuery = useQuery({
    ...
    queryFn: () => listEstoqueService(filtersState),
  });

  const movimentacoesQuery = useQuery({
    queryKey: estoqueQueryKeys.movimentacoes(JSON.stringify(toMovimentacaoFilters(filtersState))),
    queryFn: () => listMovimentacoesEstoqueService(filtersState),
  });
  ...
  return (
    <section className="space-y-4">
      <PageTitle
        title="Estoque"
        description="Controle de saldos, filtros e histórico de movimentações."
      ...
      <PageTitle title="Histórico de movimentações" description="Movimentações mais recentes registradas no sistema." />
      ...
      {movimentacoesQuery.isError ? (
        <Card>
          <CardContent className="p-6 text-sm text-destructive">Erro ao carregar histórico de movimentações.</CardContent>
        </Card>
      ) : null}

      {movimentacoesQuery.data && movimentacoesQuery.data.length === 0 ? (
        <Card>
          <CardContent className="p-6 text-sm text-muted-foreground">Nenhuma movimentação registrada.</CardContent>
        </Card>
      ) : null}
