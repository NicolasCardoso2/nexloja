...
type ListProdutosFilters = { query?: string; categoriaId?: number; ativo?: boolean };

export async function listProdutosRepository(filters?: ListProdutosFilters): Promise<ProdutoListItemEntity[]> {
  const produtos = await apiListarProdutos(filters);
