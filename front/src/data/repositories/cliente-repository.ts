...
type ListClientesFilters = { query?: string; ativo?: boolean };

export async function listClientesRepository(filters?: ListClientesFilters): Promise<ClienteListItemEntity[]> {
  const clientes = await apiListarClientes(filters);
